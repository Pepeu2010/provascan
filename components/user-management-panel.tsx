...
function ManagedUserCard({ user, currentUserId, currentRole, onChanged }: { user: ManagedUser; currentUserId: string; currentRole: string; onChanged: (message: string) => Promise<void> }) {
  ...
  return <div className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-base font-semibold text-[var(--foreground)]">{user.nome}</p><Badge tone={active ? "success" : "error"}>{active ? "Ativo" : "Desativado"}</Badge><Badge tone={roleTone(user.perfil)}>{roleLabel(user.perfil)}</Badge>{self ? <Badge tone="neutral">Sua conta</Badge> : null}{!self && !canManage ? <Badge tone="neutral">Somente leitura</Badge> : null}</div><p className="mt-2 text-sm text-[var(--muted-foreground)]">{user.email} · {user.trocar_senha.toUpperCase() === "SIM" ? "troca de senha pendente" : "senha regular"}</p></div>{canManage ? <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={busy} onClick={() => void request({ action: "reset-mfa" })}><KeyRound className="size-4" />Resetar MFA</Button>{active ? <Button variant={confirmDeactivate ? "danger" : "ghost"} disabled={busy} onClick={() => confirmDeactivate ? void request({ action: "set-active", active: false }) : setConfirmDeactivate(true)}>{confirmDeactivate ? <><ShieldOff className="size-4" />Confirmar desativação</> : <><ShieldOff className="size-4" />Desativar</>}</Button> : <Button disabled={busy} onClick={() => void request({ action: "set-active", active: true })}><ShieldCheck className="size-4" />Reativar</Button>}</div> : null}</div>{canManage ? <details className="mt-4 border-t border-[var(--border)] pt-4"><summary className="flex cursor-pointer items-center gap-2 text-sm font-semibold text-[var(--foreground)]"><Pencil className="size-4 text-[var(--accent)]" />Editar dados e perfil</summary><div className="mt-4 grid gap-3 md:grid-cols-3"><Input value={name} onChange={(event) => setName(event.target.value)} aria-label={`Nome de ${user.nome}`} /><Input value={access} onChange={(event) => setAccess(event.target.value)} aria-label={`Acesso de ${user.nome}`} /><Select value={role} onChange={(event) => setRole(event.target.value as Role)} aria-label={`Perfil de ${user.nome}`}>{assignableRoles.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></div><Button className="mt-3" variant="secondary" loading={busy} onClick={() => void request({ action: "update", nome: name, acesso: access, perfil: role })}><Pencil className="size-4" />Salvar alterações</Button></details> : null}</div>;
}
