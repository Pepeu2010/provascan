...
import {
  ...
  Copy, Download, Eye, FileText, FileUp, Filter, GripVertical, Image as ImageIcon, LayoutGrid, List, MoreHorizontal,
...
import type { ExamLifecycleStatus, ExamQuestionType, TeacherExam, TeacherExamContentVersion, TeacherExamInput } from "@/types/teacher-exams";
...
export function TeacherExamsWorkspace({ libraryOnly = false }: { libraryOnly?: boolean }) {
  ...
  async function restoreContentVersion(versionId: string) {
    if (!active || busy || !canCreateExam || active.creatorId !== viewerId) return;
    if (!window.confirm("Restaurar este conteúdo? A aplicação atual da prova será mantida.")) return;
    setBusy(true);
    setMessage("Restaurando conteúdo…");
    try {
      const result = await jsonApi<{ message: string }>(`/api/teacher-exams/${encodeURIComponent(active.id)}/versions/${encodeURIComponent(versionId)}/restore`, {
        method: "POST",
        body: JSON.stringify({ expectedVersion: active.version }),
      });
      setMessage(result.message);
      await load(active.id);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Não foi possível restaurar esta versão."); }
    finally { setBusy(false); }
  }
  ...
  return (
    <div className="teacher-exams mx-auto grid max-w-[1420px] gap-5">
      ...
      <div className="teacher-exams__viewbar"><span>{filtered.length} {filtered.length === 1 ? "prova encontrada" : "provas encontradas"}</span><div role="group" aria-label="Visualização das provas"><button type="button" aria-pressed={viewMode === "cards"} onClick={() => setViewMode("cards")}><LayoutGrid className="size-4" />Cartões</button><button type="button" aria-pressed={viewMode === "compact"} onClick={() => setViewMode("compact")}><List className="size-4" />Compacta</button></div></div>

      {loading ? <ExamListSkeleton /> : filtered.length ? <section className={`teacher-exams__grid ${viewMode === "compact" ? "is-compact" : ""}`} aria-label="Provas encontradas">{filtered.map((exam) => <ExamRow key={exam.id} exam={exam} busy={busy} readOnly={!canCreateExam || exam.creatorId !== viewerId} onAction={action} onEdit={() => openEditor(exam)} onPreparePrint={() => openEditor(exam, "revisao")} />)}</section> : <EmptyLibrary filtered={exams.length > 0} readOnly={!canCreateExam} onCreate={() => setMode("escolha")} />}
    </div>
  );
}
...
function ExamEditor({ active, autoSavedAt, busy, draft, message, readOnly, step, setDraft, setStep, viewerId, onBack, onRestoreVersion, onSave, dragIndex, setDragIndex }: {
  ...
  setDraft: React.Dispatch<React.SetStateAction<TeacherExamInput>>; setStep: (value: EditorStep) => void; viewerId: string; onBack: () => void; onRestoreVersion: (versionId: string) => Promise<void>; onSave: (intent: "rascunho" | "publicar") => Promise<void>; dragIndex: number | null; setDragIndex: (index: number | null) => void;
}) {
  ...
  const completedSteps: Record<EditorStep, boolean> = {
    informacoes: Boolean(draft.title.trim() || draft.subject.trim()),
    questoes: draft.questions.length > 0 && draft.questions.every((question) => question.prompt.trim() || question.annulled),
    gabarito: draft.questions.length > 0 && draft.questions.every((question) => question.annulled || !["multipla_escolha", "verdadeiro_falso"].includes(question.type) || question.correctAnswers.length > 0),
    aplicacao: Boolean(draft.audienceLabel || draft.assignmentGroups?.some((group) => group.classIds.length)),
    revisao: publicationErrors.length === 0,
  };
  ...
  return <div className="exam-studio"><header className="exam-studio__header"><Button variant="ghost" onClick={onBack}><ArrowLeft className="size-4" />Voltar</Button><div className="min-w-0 flex-1"><p className="teacher-exams__eyebrow">{active ? `VERSÃO ${active.version}` : "NOVA PROVA"}</p><h1>{draft.title || "Prova sem título"}</h1><p>{readOnly ? `Criada por ${active?.creatorName}` : autoSavedAt ? `Edição protegida neste aparelho às ${autoSavedAt}` : "Seu progresso é salvo neste aparelho"}</p></div>{active ? <Badge tone={statusTones[active.status]}>{statusLabels[active.status]}</Badge> : <Badge tone="warning">Novo rascunho</Badge>}</header>
    ...
    <nav ref={stepsRef} className="exam-studio__steps" aria-label="Etapas da criação">{steps.map((item, index) => <button type="button" key={item.id} aria-current={step === item.id ? "step" : undefined} aria-label={`${index + 1}. ${item.label}: ${step === item.id ? "etapa atual" : completedSteps[item.id] ? "concluída" : "pendente"}`} onClick={() => goToStep(item.id)} className={step === item.id ? "is-active" : completedSteps[item.id] ? "is-complete" : ""}><span aria-hidden="true">{completedSteps[item.id] && step !== item.id ? <Check className="size-3" /> : index + 1}</span><strong>{item.label}</strong></button>)}</nav>
    {message ? <p className="teacher-exams__message whitespace-pre-line" role="status" aria-live="polite">{message}</p> : null}
    <div className="exam-studio__layout"><Card className="exam-studio__canvas">
      ...
      {step === "revisao" ? <><ReviewSummary draft={draft} audience={audience} onEdit={goToStep} /><ExamReview draft={draft} errors={publicationErrors} active={active} audience={audience} readOnly={readOnly} onRestoreVersion={onRestoreVersion} /></> : null}
...
function BasicInformation({ draft, readOnly, setDraft }: { draft: TeacherExamInput; readOnly: boolean; setDraft: React.Dispatch<React.SetStateAction<TeacherExamInput>> }) {
  ...
  const normalizedYear = yearInput.trim() ? normalizeYearSegment(yearInput) : "";
...
function ReviewSummary({ draft, audience, onEdit }: { draft: TeacherExamInput; audience: ExamAudience; onEdit: (step: EditorStep) => void }) {
  const answered = draft.questions.filter((question) => question.annulled || question.correctAnswers.length > 0).length;
  const assignments = (draft.assignmentGroups ?? []).reduce((total, group) => total + group.classIds.length, 0);
  const rows: Array<{ step: EditorStep; title: string; detail: string }> = [
    { step: "informacoes", title: "Informações", detail: `${draft.title.trim() || "Nome não informado"} · ${draft.subject.trim() || "Disciplina não informada"}` },
    { step: "questoes", title: "Questões", detail: `${draft.questions.length} ${draft.questions.length === 1 ? "questão" : "questões"} nesta prova` },
    { step: "gabarito", title: "Gabarito", detail: `${answered} de ${draft.questions.length} respostas definidas ou anuladas` },
    { step: "aplicacao", title: "Aplicação", detail: assignments ? `${assignments} ${assignments === 1 ? "vínculo" : "vínculos"} professor–turma` : draft.audienceLabel || (audience.classes.length ? "Não definida" : "Sem turma definida") },
  ];
  return <section className="exam-review-summary" aria-label="Conferir antes de publicar"><h2>Confira antes de publicar</h2><p>Você pode voltar a qualquer etapa sem perder o que já escreveu.</p><dl>{rows.map((row) => <div key={row.step}><dt>{row.title}</dt><dd>{row.detail}</dd><button type="button" onClick={() => onEdit(row.step)} aria-label={`Alterar ${row.title.toLowerCase()}`}>Alterar</button></div>)}</dl></section>;
}

function ExamReview({ draft, errors, active, audience, readOnly, onRestoreVersion }: { draft: TeacherExamInput; errors: string[]; active: TeacherExam | null; audience: ExamAudience; readOnly: boolean; onRestoreVersion: (versionId: string) => Promise<void> }) {
...
function ExamCenter({ exam, readOnly, onRestoreVersion }: { exam: TeacherExam; readOnly: boolean; onRestoreVersion: (versionId: string) => Promise<void> }) {
  ...
  useEffect(() => {
    let current = true;
    void jsonApi<{ versions: TeacherExamContentVersion[] }>(`/api/teacher-exams/${encodeURIComponent(exam.id)}/versions`)
      .then((payload) => { if (current) { setVersions(payload.versions); setVersionsState("ready"); } })
      .catch(() => { if (current) setVersionsState("error"); });
    return () => { current = false; };
  }, [exam.id]);
