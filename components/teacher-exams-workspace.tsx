...
function CreationChoice({ busy, fileInput, onBack, onImport, onManual }: { busy: boolean; fileInput: React.RefObject<HTMLInputElement | null>; onBack: () => void; onImport: (file: File) => Promise<void>; onManual: () => void }) {
  return <div className="mx-auto grid max-w-[980px] gap-6"><Button variant="ghost" className="w-fit" onClick={onBack}><ArrowLeft className="size-4" />Voltar para provas</Button><header className="text-center"><p className="teacher-exams__eyebrow">NOVA PROVA</p><h1 className="mt-3 text-3xl font-bold tracking-[-0.04em] sm:text-4xl">Como você quer começar?</h1><p className="mx-auto mt-3 max-w-2xl text-[var(--muted-foreground)]">Escolha o caminho mais rápido. Tudo continua editável antes da publicação.</p></header><div className="grid gap-4 md:grid-cols-2"><button type="button" className="creation-choice" onClick={onManual}><span className="creation-choice__icon"><Pencil className="size-6" /></span><strong>Criar manualmente</strong><p>Preencha as informações, escreva as questões e configure o gabarito passo a passo.</p><span>Começar em branco <ChevronRight className="size-4" /></span></button><button type="button" className="creation-choice" disabled={busy} onClick={() => fileInput.current?.click()}><span className="creation-choice__icon"><UploadCloud className="size-6" /></span><strong>{busy ? "Preparando revisão…" : "Importar arquivo"}</strong><p>Envie um PDF, Word ou imagem para transformar o conteúdo em uma prova editável.</p><span>PDF, DOC, DOCX, JPG ou PNG · até 15 MB <ChevronRight className="size-4" /></span></button></div><input ref={fileInput} className="sr-only" type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onChange={(event) => { const file = event.target.files?.[0]; if (file) void onImport(file); }} /><aside className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-sm text-[var(--muted-foreground)]"><strong className="text-[var(--foreground)]">Na importação:</strong> enviamos o original para armazenamento privado, validamos o formato real e preparamos uma revisão. Nada é publicado automaticamente.</aside></div>;
}
