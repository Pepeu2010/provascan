...
function ExamEditor({ active, autoSavedAt, busy, draft, message, readOnly, step, setDraft, setStep, viewerId, onBack, onSave, dragIndex, setDragIndex }: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const suffix = draft.subjectId ? `?subjectId=${encodeURIComponent(draft.subjectId)}` : "";
        const result = await jsonApi<Partial<ExamAudience>>(`/api/teacher-exams/audience${suffix}`);
  ...
  return <div className="exam-studio"><header className="exam-studio__header"><Button variant="ghost" onClick={onBack}><ArrowLeft className="size-4" />Voltar</Button><div className="min-w-0 flex-1"><p className="teacher-exams__eyebrow">{active ? `VERSÃO ${active.version}` : "NOVA PROVA"}</p><h1>{draft.title || "Prova sem título"}</h1><p>{readOnly ? `Criada por ${active?.creatorName}` : autoSavedAt ? `Edição protegida neste aparelho às ${autoSavedAt}` : "Seu progresso é salvo neste aparelho"}</p></div>{active ? <Badge tone={statusTones[active.status]}>{statusLabels[active.status]}</Badge> : <Badge tone="warning">Novo rascunho</Badge>}</header>
    ...
    <div className="exam-studio__layout"><Card className="exam-studio__canvas">
      {step === "informacoes" ? <BasicInformation data={data} draft={draft} readOnly={readOnly} setDraft={setDraft} subjects={audience.subjects} subjectsLoading={audienceLoading} /> : null}
...
function BasicInformation({ data, draft, readOnly, setDraft, subjects, subjectsLoading }: { data: ReturnType<typeof useAppData>["data"]; draft: TeacherExamInput; readOnly: boolean; setDraft: React.Dispatch<React.SetStateAction<TeacherExamInput>>; subjects: ExamAudience["subjects"]; subjectsLoading: boolean }) {
  const set = (changes: Partial<TeacherExamInput>) => setDraft((current) => ({ ...current, ...changes }));
  const selectTypedSubject = (value: string) => {
    const normalized = value.trim().toLocaleLowerCase("pt-BR");
    const selected = subjects.find((item) => item.name.trim().toLocaleLowerCase("pt-BR") === normalized);
    const nextSubjectId = selected?.id ?? null;
    set({
      assignmentGroups: nextSubjectId === draft.subjectId ? draft.assignmentGroups : [],
      audienceId: nextSubjectId === draft.subjectId ? draft.audienceId : "",
      audienceLabel: nextSubjectId === draft.subjectId ? draft.audienceLabel : "",
      subject: value,
      subjectId: nextSubjectId,
    });
  };

  return <section><SectionHeading eyebrow="ETAPA 1" title="Informações da prova" detail="Digite a disciplina da prova. Na próxima etapa, defina quem aplicará a prova." /><div className="exam-form"><label className="sm:col-span-2">Nome da prova<Input disabled={readOnly} value={draft.title} onChange={(event) => set({ title: event.target.value })} placeholder="Ex.: Avaliação de Matemática — 1º Bimestre" /></label><label>Disciplina<Input disabled={readOnly} list="exam-subject-suggestions" value={draft.subject} onChange={(event) => selectTypedSubject(event.target.value)} placeholder={subjectsLoading ? "Carregando sugestões…" : "Ex.: Matemática"} aria-describedby="exam-subject-help" /><datalist id="exam-subject-suggestions">{subjects.map((item) => <option key={item.id} value={item.name} />)}</datalist><small id="exam-subject-help">{draft.subject && !draft.subjectId ? "Use uma disciplina cadastrada para definir turmas e publicar." : "Digite livremente; as disciplinas cadastradas aparecem como sugestão."}</small></label><label>Turma principal<Select disabled={readOnly} value={draft.audienceId} onChange={(event) => { const selected = data.classes.find((item) => item.id === event.target.value); set({ audienceId: selected?.id ?? "", audienceLabel: selected?.nome ?? "", groupType: selected?.groupType ?? "TURMA", yearSegment: selected?.yearSegment ?? "OUTROS" }); }}><option value="">Defina na etapa Aplicação</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</Select></label><label>Série/Ano<Input disabled={readOnly} value={draft.yearSegment} onChange={(event) => set({ yearSegment: event.target.value })} placeholder="1º ano" /></label><label>Bimestre/Trimestre<Input disabled={readOnly} value={draft.period} onChange={(event) => set({ period: event.target.value })} placeholder="1º Bimestre" /></label><label>Data prevista<Input disabled={readOnly} type="date" value={draft.examDate} onChange={(event) => set({ examDate: event.target.value })} /></label><label>Tempo estimado (minutos)<Input disabled={readOnly} type="number" min={1} max={600} value={draft.estimatedDuration ?? ""} onChange={(event) => set({ estimatedDuration: event.target.value ? Number(event.target.value) : null })} /></label><label className="sm:col-span-2">Descrição opcional<Textarea disabled={readOnly} value={draft.description} onChange={(event) => set({ description: event.target.value })} placeholder="Contexto ou observação para sua organização" /></label><label className="sm:col-span-2">Instruções para os alunos<Textarea disabled={readOnly} value={draft.instructions} onChange={(event) => set({ instructions: event.target.value })} placeholder="Leia com atenção e marque apenas uma alternativa…" /></label></div></section>;
}
...
function AnswerKeyEditor({ draft, readOnly, setDraft, updateQuestion, totalWeight }: { draft: TeacherExamInput; readOnly: boolean; setDraft: React.Dispatch<React.SetStateAction<TeacherExamInput>>; updateQuestion: (index: number, changes: Partial<TeacherExamInput["questions"][number]>) => void; totalWeight: number }) {
  ...
  return <section><SectionHeading eyebrow="ETAPA 3" title="Gabarito e pontuação" detail="Use os botões ou cole a sequência de letras do gabarito. Você não precisa abrir um seletor para cada questão." /><div className="answer-key-summary"><span><strong>{draft.questions.length}</strong> questões</span><span><strong>{totalWeight.toLocaleString("pt-BR")}</strong> pontos</span><span><strong>{draft.questions.filter((question) => question.annulled).length}</strong> anuladas</span></div>{!readOnly && objectiveCount ? <div className="answer-key-batch"><div><strong>Colar gabarito em sequência</strong><span>Ex.: <b>A B C D</b> ou <b>ABCD</b> · {objectiveCount} questões objetivas ativas.</span></div><label><span className="sr-only">Sequência de respostas corretas</span><Textarea rows={2} value={sequence} onChange={(event) => { setSequence(event.target.value); setSequenceMessage(""); }} placeholder="A B C D A B..." /></label><div><span>{sequenceAnswers.length} {sequenceAnswers.length === 1 ? "letra reconhecida" : "letras reconhecidas"}</span><Button variant="secondary" disabled={!sequenceAnswers.length} onClick={applySequence}><Check className="size-4" />Aplicar sequência</Button></div>{sequenceMessage ? <p role="status" aria-live="polite">{sequenceMessage}</p> : null}</div> : null}<div className="grid gap-3">{draft.questions.map((question, index) => { const objective = question.type === "multipla_escolha" || question.type === "verdadeiro_falso"; return <article key={question.id ?? index} className="answer-key-row"><div><strong>Q{index + 1}</strong><span>{typeLabels[question.type]}</span></div>{objective ? <div className="answer-key-row__choices" role="group" aria-label={`Resposta correta da questão ${index + 1}`}>{question.alternatives.map((alternative, alternativeIndex) => <button key={`${alternative}-${alternativeIndex}`} type="button" disabled={readOnly || question.annulled || !alternative.trim()} aria-pressed={question.correctAnswers[0] === alternative} className={question.correctAnswers[0] === alternative ? "is-selected" : ""} onClick={() => updateQuestion(index, { correctAnswers: [alternative], needsReview: false })}>{String.fromCharCode(65 + alternativeIndex)}</button>)}{!question.alternatives.some((alternative) => alternative.trim()) ? <span>Preencha as alternativas na etapa Questões.</span> : null}</div> : <span className="answer-key-row__manual">Correção manual por critério</span>}<label className="answer-key-row__weight">Valor<Input disabled={readOnly} type="number" min={0} step="0.1" value={question.weight} onChange={(event) => updateQuestion(index, { weight: Number(event.target.value) })} /></label><Checkbox disabled={readOnly} checked={question.annulled} label="Anulada" onChange={(event) => updateQuestion(index, { annulled: event.target.checked, correctAnswers: event.target.checked ? [] : question.correctAnswers })} /></article>; })}</div></section>;
}
...
function ApplicationEditor({ audience, draft, loading, readOnly, setDraft, viewerId }: { audience: ExamAudience; draft: TeacherExamInput; loading: boolean; readOnly: boolean; setDraft: React.Dispatch<React.SetStateAction<TeacherExamInput>>; viewerId: string }) {
  ...
  return <section><SectionHeading eyebrow="ETAPA 4" title="Aplicação da prova" detail="Cada seleção une um professor responsável a uma turma específica. Nada é cruzado automaticamente." />
    {!hasSubject ? <div className="publish-checklist review-errors"><CircleAlert className="size-5" /><div><strong>Escolha a disciplina primeiro</strong><p>As turmas e responsáveis aparecem conforme o escopo autorizado.</p></div></div> : loading ? <p className="text-sm text-[var(--muted-foreground)]">Carregando professores e turmas permitidos…</p> : !audience.teachers.length ? <div className="publish-checklist review-errors"><CircleAlert className="size-5" /><div><strong>Nenhuma atribuição disponível</strong><p>Não há escopo pedagógico ativo para esta disciplina.</p></div></div> : <div className="application-groups">{audience.teachers.map((teacher) => <article className="application-group" key={teacher.id}><header><strong>{teacher.id === viewerId ? "Você" : teacher.name}</strong><span>{teacher.id === viewerId ? "Responsável pela aplicação" : "Professor autorizado"}</span></header><div>{teacher.classIds.map((classId) => { const classroom = audience.classes.find((item) => item.id === classId); if (!classroom) return null; const selected = groups.some((group) => group.teacherId === teacher.id && group.classIds.includes(classId)); return <Checkbox key={classId} disabled={readOnly} checked={selected} label={classroom.name} onChange={(event) => change(teacher.id, classId, event.target.checked)} />; })}</div></article>)}</div>}
  </section>;
}
