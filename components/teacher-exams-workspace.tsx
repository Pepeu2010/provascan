...
function ExamEditor({ active, autoSavedAt, busy, draft, message, readOnly, step, setDraft, setStep, viewerId, onBack, onSave, dragIndex, setDragIndex }: {
  ...
  return <div className="exam-studio"><header className="exam-studio__header"><Button variant="ghost" onClick={onBack}><ArrowLeft className="size-4" />Voltar</Button><div className="min-w-0 flex-1"><p className="teacher-exams__eyebrow">{active ? `VERSÃO ${active.version}` : "NOVA PROVA"}</p><h1>{draft.title || "Prova sem título"}</h1><p>{readOnly ? `Criada por ${active?.creatorName}` : autoSavedAt ? `Edição protegida neste aparelho às ${autoSavedAt}` : "Seu progresso é salvo neste aparelho"}</p></div>{active ? <Badge tone={statusTones[active.status]}>{statusLabels[active.status]}</Badge> : <Badge tone="warning">Novo rascunho</Badge>}</header>
    ...
    <div className="exam-studio__layout"><Card className="exam-studio__canvas">
      {step === "informacoes" ? <BasicInformation draft={draft} readOnly={readOnly} setDraft={setDraft} subjects={audience.subjects} subjectsLoading={audienceLoading} /> : null}
...
function BasicInformation({ draft, readOnly, setDraft, subjects, subjectsLoading }: { draft: TeacherExamInput; readOnly: boolean; setDraft: React.Dispatch<React.SetStateAction<TeacherExamInput>>; subjects: ExamAudience["subjects"]; subjectsLoading: boolean }) {
  ...
  return <section><SectionHeading eyebrow="ETAPA 1" title="Comece pelo essencial" detail="Preencha apenas o que ajudar agora. Você pode publicar sem cadastrar disciplina, turma ou período." /><div className="exam-form"><label className="sm:col-span-2">Nome da prova <small>(opcional)</small><Input disabled={readOnly} value={draft.title} onChange={(event) => set({ title: event.target.value })} placeholder="Ex.: Avaliação do 2º ano" /></label><label>Disciplina <small>(opcional)</small><Input disabled={readOnly} list="exam-subject-suggestions" value={draft.subject} onChange={(event) => typeSubject(event.target.value)} placeholder={subjectsLoading ? "Carregando sugestões…" : "Ex.: Matemática"} aria-describedby="exam-subject-help" /><datalist id="exam-subject-suggestions">{subjects.map((item) => <option key={item.id} value={item.name} />)}</datalist><small id="exam-subject-help">Escreva como preferir. Isso fica somente nesta prova; não cria um cadastro novo.</small></label><label>Série ou ano <small>(opcional)</small><Input disabled={readOnly} value={yearInput} onChange={(event) => updateYear(event.target.value)} placeholder="Ex.: 2º ano" /><small>Ao informar o ano, a prova fica pronta para todo esse ano.</small></label><details className="exam-form__optional sm:col-span-2"><summary>Adicionar mais informações (opcional)</summary><div><label>Bimestre ou trimestre<Input disabled={readOnly} value={draft.period} onChange={(event) => set({ period: event.target.value })} placeholder="Ex.: 1º bimestre" /></label><label>Data prevista<Input disabled={readOnly} type="date" value={draft.examDate} onChange={(event) => set({ examDate: event.target.value })} /></label><label>Tempo estimado (minutos)<Input disabled={readOnly} type="number" min={1} max={600} value={draft.estimatedDuration ?? ""} onChange={(event) => set({ estimatedDuration: event.target.value ? Number(event.target.value) : null })} /></label><label className="sm:col-span-2">Descrição<Textarea disabled={readOnly} value={draft.description} onChange={(event) => set({ description: event.target.value })} placeholder="Uma anotação para sua organização" /></label><label className="sm:col-span-2">Instruções para os alunos<Textarea disabled={readOnly} value={draft.instructions} onChange={(event) => set({ instructions: event.target.value })} placeholder="Leia com atenção e marque apenas uma alternativa…" /></label></div></details></div></section>;
}
