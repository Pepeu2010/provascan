...
function printAnswerKey(exam: CollaborativeExam) {
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8" /><title>Gabarito — ${escapeForHtml(exam.title)}</title><style>
    ...
    * { box-sizing: border-box; } body { min-width: 210mm; margin: 0; padding: 12mm 0; background: #e9edf4; color: #172033; font-family: Arial, Helvetica, sans-serif; } .print-sheet { width: 210mm; min-height: 297mm; margin: 0 auto; padding: 20mm 23mm; background: #fff; box-shadow: 0 14px 38px rgb(26 37 56 / 18%); }
    ...
    h2 { margin: 28px 0 13px; font-size: 13px; text-align: center; text-transform: uppercase; letter-spacing: .1em; } .answers { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 12px; max-width: 164mm; margin: 0 auto; padding: 11mm; border: 1px solid #d5ddea; border-radius: 16px; background: #f8faff; } .answer { display: grid; min-height: 27mm; place-items: center; gap: 6px; border: 1.5px solid #bdc9dc; border-radius: 12px; padding: 10px; background: #fff; break-inside: avoid; } .answer span { color: #667085; font-size: 12px; font-weight: 700; } .answer strong { color: #5538c8; font-size: 27px; }
...
function printStudentCards(exam: CollaborativeExam, students: Student[], classNames: Map<string, string>) {
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Cartões — ${escapeForHtml(exam.title)}</title><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}body{min-width:210mm;margin:0;padding:12mm 0;background:#e9edf4;color:#172033;font-family:Arial,Helvetica,sans-serif}.card{width:210mm;min-height:297mm;margin:0 auto 12mm;padding:20mm 23mm;background:#fff;box-shadow:0 14px 38px rgb(26 37 56 / 18%);break-after:page;page-break-after:always}.card:last-child{margin-bottom:0;break-after:auto;page-break-after:auto}header{display:flex;justify-content:space-between;gap:20px;border-bottom:3px solid #5538c8;padding-bottom:12px}header b{color:#5538c8;font-size:10px;letter-spacing:.1em}h1{margin:6px 0;font-size:22px}p{margin:0;color:#526075;font-size:12px}.student{min-width:180px;border-left:1px solid #d5dbea;padding-left:14px}.student small{display:block;color:#667085;font-size:9px;letter-spacing:.1em}.student strong{display:block;margin-top:5px;font-size:14px}.instruction{margin:17px 0 10px;text-align:center}.answer-space{display:flex;width:164mm;min-height:155mm;flex-direction:column;justify-content:center;margin:18mm auto 0;padding:12mm 14mm;border:1.5px solid #cfd9e8;border-radius:18px;background:#f8faff}.question{display:grid;grid-template-columns:42px repeat(${exam.alternatives.length},28px 32px);align-items:center;justify-content:center;gap:8px;min-height:12mm;border-bottom:1px solid #dce4ef;font-size:12px}.question:last-child{border-bottom:0}.question b{font-size:15px}.question span{text-align:center;font-weight:700}.question i{width:24px;height:24px;border:2px solid #172033;border-radius:50%}footer{margin-top:18px;color:#667085;font-size:10px;text-align:center}@media print{body{padding:0;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.card{margin:0;box-shadow:none}}</style></head><body>${cards}</body></html>`;
  printWindow.document.open();
  printWindow.document.write(html);
  const requestPrint = () => {
    printWindow.focus();
      ...
      {management ? (
        <div className="mt-6 grid gap-4" aria-busy={isCreating}>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Select disabled={isCreating} value={audienceId} onChange={(event) => setAudienceId(event.target.value)}><option value="">Aplicação geral</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</Select>
