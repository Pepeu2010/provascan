...
function printAnswerKey(exam: CollaborativeExam) {
  ...
  </style></head><body><div class="print-sheet"><header><div class="eyebrow">PROVASCAN · GABARITO FINAL</div><h1>${escapeForHtml(exam.title)}</h1><div class="meta">${escapeForHtml(exam.audienceLabel)} · ${escapeForHtml(formatPrintDate(exam.examDate))} · ${exam.questionCount} questões</div></header><div class="summary"><div><b>ORDEM DE IMPRESSÃO</b><span>Questões 1 a ${exam.questionCount}</span></div><div><b>SITUAÇÃO</b><span>Gabarito liberado</span></div></div><h2>Respostas em ordem numérica</h2><main class="answers">${answerRows}</main><h2>Blocos da prova</h2><ul>${sectionRows}</ul><footer>Gerado pelo ProvaScan. Confira este documento antes de distribuir aos aplicadores.</footer></div></body></html>`;
...
function printStudentCards(exam: CollaborativeExam, students: Student[], classNames: Map<string, string>) {
  ...
  const sortedSections = [...exam.sections].sort((left, right) => left.questionStart - right.questionStart);
  const sectionStarts = new Set(sortedSections.slice(1).map((section) => section.questionStart));
  const layout = getStudentCardPrintLayout(exam.questionCount, exam.alternatives);
  const columnHeaders = `<div class="question-header" aria-hidden="true"><span>Nº</span>${exam.alternatives.map((alternative) => `<span>${escapeForHtml(alternative)}</span>`).join("")}</div>`;
  const questionColumns = Array.from({ length: layout.columnCount }, (_, columnIndex) => {
    const firstQuestionIndex = columnIndex * layout.rowsPerColumn;
    const questionsInColumn = Math.min(layout.rowsPerColumn, exam.questionCount - firstQuestionIndex);
    const rows = Array.from({ length: Math.max(questionsInColumn, 0) }, (_, rowIndex) => {
      const question = firstQuestionIndex + rowIndex + 1;
      return `<div class="question${sectionStarts.has(question) ? " question--section-start" : ""}"><b>${question}</b>${exam.alternatives.map(() => `<i aria-hidden="true"></i>`).join("")}</div>`;
    }).join("");
    return `<section class="question-column" aria-label="Questões ${firstQuestionIndex + 1} a ${firstQuestionIndex + Math.max(questionsInColumn, 0)}">${columnHeaders}${rows}</section>`;
  }).join("");
  const subjectMap = sortedSections.length >= 2 ? `<div class="subject-map" aria-label="Divisão das matérias">${sortedSections.map((section) => `<div style="--subject-weight:${section.questionCount}"><strong>${escapeForHtml(section.subject)}</strong><span>${section.questionCount} questões · ${section.questionStart}–${section.questionEnd}</span></div>`).join("")}</div>` : "";
  const cards = students.map((student) => `<section class="card"><header><div><b>PROVASCAN · CARTÃO-RESPOSTA</b><h1>${escapeForHtml(exam.title)}</h1><p>${escapeForHtml(classNames.get(student.turma) ?? exam.audienceLabel)} · ${escapeForHtml(formatPrintDate(exam.examDate))}</p></div><div class="student"><small>ALUNO(A)</small><strong>${escapeForHtml(student.nome)}</strong></div></header><p class="instruction">Preencha completamente somente um círculo por questão.</p>${subjectMap}<main class="answer-space">${questionColumns}</main><footer>Não dobre este cartão. Use caneta azul ou preta.</footer></section>`).join("");
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Cartões — ${escapeForHtml(exam.title)}</title><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}body{min-width:210mm;margin:0;padding:12mm 0;background:#e9edf4;color:#172033;font-family:Arial,Helvetica,sans-serif}.card{position:relative;width:210mm;height:297mm;overflow:hidden;margin:0 auto 12mm;padding:20mm 17mm;background:#fff;box-shadow:0 14px 38px rgb(26 37 56 / 18%);break-after:page;page-break-after:always}.card:last-child{margin-bottom:0;break-after:auto;page-break-after:auto}header{display:flex;justify-content:space-between;gap:20px;border-bottom:3px solid #5538c8;padding-bottom:12px}header b{color:#5538c8;font-size:10px;letter-spacing:.1em}h1{margin:6px 0;font-size:22px}p{margin:0;color:#526075;font-size:12px}.student{min-width:180px;border-left:1px solid #d5dbea;padding-left:14px}.student small{display:block;color:#667085;font-size:9px;letter-spacing:.1em}.student strong{display:block;margin-top:5px;font-size:14px}.instruction{position:absolute;top:61mm;left:17mm;right:17mm;text-align:center}.subject-map{position:absolute;top:69mm;left:${layout.answerAreaLeft.toFixed(3)}mm;width:${layout.answerAreaWidth.toFixed(3)}mm;display:flex;overflow:hidden;border:1px solid #cfc7f5;border-radius:8px;background:#f4f1ff}.subject-map div{min-width:0;flex:var(--subject-weight) 1 0;border-right:1px solid #cfc7f5;padding:1.8mm 2.2mm}.subject-map div:last-child{border-right:0}.subject-map strong,.subject-map span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.subject-map strong{color:#5538c8;font-size:9px}.subject-map span{margin-top:1px;color:#526075;font-size:7px}.answer-space{position:absolute;top:${layout.answerAreaTop.toFixed(3)}mm;left:${layout.answerAreaLeft.toFixed(3)}mm;display:grid;grid-template-columns:repeat(${layout.columnCount},minmax(0,1fr));column-gap:${layout.columnGap.toFixed(3)}mm;width:${layout.answerAreaWidth.toFixed(3)}mm;height:${layout.answerAreaHeight.toFixed(3)}mm;border:1.5px solid #c5d0e0;border-radius:12px;background:#fbfcff}.question-column{position:relative;display:grid;grid-template-rows:repeat(${layout.rowsPerColumn},minmax(0,1fr));min-width:0}.question-column:not(:first-child)::before{position:absolute;top:0;bottom:0;left:${(-layout.columnGap / 2).toFixed(3)}mm;width:1px;background:#b8c4d6;content:""}.question-header{position:absolute;top:-6mm;left:0;display:grid;grid-template-columns:${layout.numberColumnWidth.toFixed(3)}mm repeat(${exam.alternatives.length},${layout.bubbleCellWidth.toFixed(3)}mm);width:100%;padding-right:${layout.rowRightPadding.toFixed(3)}mm;color:#526075;font-size:8px;font-weight:700}.question-header span{text-align:center}.question{display:grid;grid-template-columns:${layout.numberColumnWidth.toFixed(3)}mm repeat(${exam.alternatives.length},${layout.bubbleCellWidth.toFixed(3)}mm);align-items:center;width:100%;min-height:0;padding-right:${layout.rowRightPadding.toFixed(3)}mm;border-bottom:1px solid #dce4ef}.question--section-start{border-top:2px solid #8b75dc}.question b{text-align:center;font-size:11px;font-variant-numeric:tabular-nums}.question i{justify-self:center;width:${layout.bubbleSize.toFixed(3)}mm;height:${layout.bubbleSize.toFixed(3)}mm;border:1.6px solid #172033;border-radius:50%;background:#fff}.question-column .question:last-child{border-bottom:0}footer{position:absolute;right:17mm;bottom:14mm;left:17mm;color:#667085;font-size:10px;text-align:center}@media print{body{padding:0;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.card{margin:0;box-shadow:none}}</style></head><body>${cards}</body></html>`;
  printWindow.document.open();
  printWindow.document.write(html);
  const requestPrint = () => {
    printWindow.focus();
