...
function printAnswerKey(exam: CollaborativeExam) {
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8" /><title>Gabarito — ${escapeForHtml(exam.title)}</title><style>
    ...
    * { box-sizing: border-box; } body { margin: 0; color: #172033; font-family: Arial, Helvetica, sans-serif; } .print-sheet { max-width: 164mm; margin: 0 auto; }
    ...
    h2 { margin: 26px 0 12px; font-size: 13px; text-transform: uppercase; letter-spacing: .08em; } .answers { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 10px; max-width: 148mm; margin: 0 auto; padding: 8mm; border: 1px solid #e0e5ee; border-radius: 14px; background: #fbfcfe; } .answer { display: grid; min-height: 18mm; place-items: center; gap: 4px; border: 1px solid #cfd7e5; border-radius: 10px; padding: 9px 10px; background: #fff; break-inside: avoid; } .answer span { color: #667085; font-size: 11px; font-weight: 700; } .answer strong { color: #5538c8; font-size: 20px; }
...
function printStudentCards(exam: CollaborativeExam, students: Student[], classNames: Map<string, string>) {
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Cartões — ${escapeForHtml(exam.title)}</title><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}body{margin:0;color:#172033;font-family:Arial,Helvetica,sans-serif}.card{width:210mm;min-height:297mm;padding:18mm;break-after:page;page-break-after:always}.card:last-child{break-after:auto;page-break-after:auto}header{display:flex;justify-content:space-between;gap:20px;border-bottom:3px solid #5538c8;padding-bottom:12px}header b{color:#5538c8;font-size:10px;letter-spacing:.1em}h1{margin:6px 0;font-size:22px}p{margin:0;color:#526075;font-size:12px}.student{min-width:180px;border-left:1px solid #d5dbea;padding-left:14px}.student small{display:block;color:#667085;font-size:9px;letter-spacing:.1em}.student strong{display:block;margin-top:5px;font-size:14px}.instruction{margin:17px 0 10px}.answer-space{width:148mm;margin:14mm auto 0;padding:9mm 11mm;border:1px solid #d5dbea;border-radius:14px;background:#fbfcfe}.question{display:grid;grid-template-columns:36px repeat(${exam.alternatives.length},24px 28px);align-items:center;justify-content:center;gap:7px;min-height:10mm;border-bottom:1px solid #e1e7f0;font-size:11px}.question:last-child{border-bottom:0}.question b{font-size:13px}.question span{text-align:center;font-weight:700}.question i{width:20px;height:20px;border:1.7px solid #172033;border-radius:50%}footer{margin-top:14px;color:#667085;font-size:10px}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>${cards}</body></html>`;
  printWindow.document.open();
  printWindow.document.write(html);
  const requestPrint = () => {
    printWindow.focus();
