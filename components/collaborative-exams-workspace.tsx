...
function printStudentCards(exam: CollaborativeExam, students: Student[], classNames: Map<string, string>) {
  ...
  const questionRows = Array.from({ length: exam.questionCount }, (_, index) => `<div class="question${sectionStarts.has(index + 1) ? " question--section-start" : ""}"><b>${index + 1}</b>${exam.alternatives.map((alternative) => `<span>${escapeForHtml(alternative)}</span><i aria-hidden="true"></i>`).join("")}</div>`).join("");
  const subjectMap = sortedSections.length >= 2 ? `<div class="subject-map" aria-label="Divisão das matérias">${sortedSections.map((section) => `<div><strong>${escapeForHtml(section.subject)}</strong><span>${section.questionCount} questões · ${section.questionStart}–${section.questionEnd}</span></div>`).join("")}</div>` : "";
  const rowHeight = Math.min(12.5, 140 / Math.max(exam.questionCount, 1));
  const cards = students.map((student) => `<section class="card"><header><div><b>PROVASCAN · CARTÃO-RESPOSTA</b><h1>${escapeForHtml(exam.title)}</h1><p>${escapeForHtml(classNames.get(student.turma) ?? exam.audienceLabel)} · ${escapeForHtml(formatPrintDate(exam.examDate))}</p></div><div class="student"><small>ALUNO(A)</small><strong>${escapeForHtml(student.nome)}</strong></div></header><p class="instruction">Preencha somente uma alternativa por questão.</p><main class="answer-space" style="--question-row-height:${rowHeight.toFixed(3)}mm">${subjectMap}${questionRows}</main><footer>Não dobre este cartão. Use caneta azul ou preta.</footer></section>`).join("");
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Cartões — ${escapeForHtml(exam.title)}</title><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}body{min-width:210mm;margin:0;padding:12mm 0;background:#e9edf4;color:#172033;font-family:Arial,Helvetica,sans-serif}.card{width:210mm;min-height:297mm;margin:0 auto 12mm;padding:20mm 17mm;background:#fff;box-shadow:0 14px 38px rgb(26 37 56 / 18%);break-after:page;page-break-after:always}.card:last-child{margin-bottom:0;break-after:auto;page-break-after:auto}header{display:flex;justify-content:space-between;gap:20px;border-bottom:3px solid #5538c8;padding-bottom:12px}header b{color:#5538c8;font-size:10px;letter-spacing:.1em}h1{margin:6px 0;font-size:22px}p{margin:0;color:#526075;font-size:12px}.student{min-width:180px;border-left:1px solid #d5dbea;padding-left:14px}.student small{display:block;color:#667085;font-size:9px;letter-spacing:.1em}.student strong{display:block;margin-top:5px;font-size:14px}.instruction{margin:21px 0 0;text-align:center}.answer-space{position:relative;display:flex;width:170mm;max-width:100%;height:172mm;overflow:hidden;flex-direction:column;justify-content:center;margin:14mm auto 0;padding:16mm 15mm;border:1.5px solid #c5d0e0;border-radius:18px;background:#f8faff}.subject-map{position:absolute;top:2.5mm;left:8mm;right:8mm;display:grid;grid-template-columns:repeat(${Math.max(sortedSections.length, 1)},minmax(0,1fr));overflow:hidden;border:1px solid #cfc7f5;border-radius:8px;background:#f4f1ff}.subject-map div{min-width:0;border-right:1px solid #cfc7f5;padding:2mm 2.5mm}.subject-map div:last-child{border-right:0}.subject-map strong,.subject-map span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.subject-map strong{color:#5538c8;font-size:9px}.subject-map span{margin-top:1px;color:#526075;font-size:7px}.question{display:grid;grid-template-columns:46px repeat(${exam.alternatives.length},30px 34px);align-items:center;justify-content:center;gap:10px;height:var(--question-row-height);min-height:var(--question-row-height);border-bottom:1px solid #dce4ef;font-size:12px}.question--section-start{border-top:2px solid #8b75dc}.question:last-child{border-bottom:0}.question b{font-size:16px}.question span{text-align:center;font-weight:700}.question i{width:26px;height:26px;border:2px solid #172033;border-radius:50%}footer{margin-top:18px;color:#667085;font-size:10px;text-align:center}@media print{body{padding:0;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.card{margin:0;box-shadow:none}}</style></head><body>${cards}</body></html>`;
  printWindow.document.open();
  printWindow.document.write(html);
  const requestPrint = () => {
    printWindow.focus();
