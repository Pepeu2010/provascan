...
export function ReportsWorkspace() {
  ...
  useEffect(() => {
    let active = true;
    void fetch("/api/external-corrections", { cache: "no-store" })
      .then(async (response) => {
        const payload = await response.json() as { corrections?: ExternalCorrectionRecord[]; error?: string };
        if (!active) return;
        if (!response.ok) setExternalHistoryError(payload.error || "Histórico externo indisponível.");
        else setExternalCorrections(payload.corrections ?? []);
      })
      .catch(() => { if (active) setExternalHistoryError("Histórico externo indisponível."); });
    return () => { active = false; };
  }, []);
