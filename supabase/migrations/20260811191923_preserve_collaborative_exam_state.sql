-- Collaborative exams have teacher-owned sections and must never be removed by
-- the legacy whole-state persistence RPC. That RPC still serves classes,
-- students and legacy exams, so isolate every record attached to a section.
...
begin
  ...
  delete from public.corrections where exam_id not in (select exam_id from public.exam_sections);
  delete from public.correction_rules where exam_id not in (select exam_id from public.exam_sections);
  delete from public.answer_keys where exam_id not in (select exam_id from public.exam_sections);
  delete from public.students where true;
  delete from public.exams where id not in (select exam_id from public.exam_sections);
  ...
  select item->>'id', item->>'nome', coalesce(item->>'ano', ''), coalesce(item->>'audienceId', ''), coalesce(item->>'audienceLabel', ''), coalesce(item->>'groupType', ''), coalesce(item->>'yearSegment', '')
  from jsonb_array_elements(coalesce(payload->'classes', '[]'::jsonb)) item;
  insert into public.students (id, name, class_id, status)
  select item->>'id', item->>'nome', nullif(item->>'turma', ''), coalesce(item->>'status', 'Ativo')
  from jsonb_array_elements(coalesce(payload->'students', '[]'::jsonb)) item;
  insert into public.exams (id, title, audience_id, audience_label, group_type, year_segment, question_count, alternatives, exam_date, code, template_version, released_at)
  select item->>'id', item->>'titulo', coalesce(item->>'audienceId', ''), coalesce(item->>'audienceLabel', ''), coalesce(item->>'groupType', ''), coalesce(item->>'yearSegment', ''), coalesce((item->>'quantidadeQuestoes')::integer, 0), coalesce(item->'alternativas', '[]'::jsonb), coalesce(item->>'data', ''), coalesce(item->>'codigo', ''), coalesce(item->>'templateVersion', 'PS-CARD-2'), nullif(item->>'releasedAt', '')::timestamptz
  from jsonb_array_elements(coalesce(payload->'exams', '[]'::jsonb)) item
  where not exists (select 1 from public.exam_sections section where section.exam_id = item->>'id');
  ...
  where not exists (select 1 from public.exam_sections section where section.exam_id = item->>'provaId');
  ...
  select item->>'provaId', coalesce((item->>'notaMaxima')::numeric, 10), coalesce((item->>'arredondamentoCasas')::integer, 1), coalesce((item->>'pesoPadrao')::numeric, 1), coalesce(item->'pesosPorQuestao', '[]'::jsonb), coalesce(item->'questoesAnuladas', '[]'::jsonb), coalesce(item->>'modoQuestaoAnulada', 'full-credit')
