-- Matrícula não participa do fluxo operacional desta escola. O identificador
-- interno segue protegendo o QR e o nome é usado na conferência manual/OCR.
...
begin
  ...
  select item->>'id', item->>'nome', coalesce(nullif(btrim(item->>'professor'), ''), 'Professor responsável não informado'), coalesce(item->>'ano', ''), coalesce(item->>'audienceId', ''), coalesce(item->>'audienceLabel', ''), coalesce(item->>'groupType', ''), coalesce(item->>'yearSegment', '') from jsonb_array_elements(coalesce(payload->'classes', '[]'::jsonb)) item;
