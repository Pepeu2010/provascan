-- A escola opera com um único período. O dado não agrega à correção e criava
-- ambiguidade na tela; cada turma passa a ter somente o professor responsável.
update public.classes
set teacher = coalesce(
  nullif(btrim(teacher), ''),
  (select nullif(btrim(value), '') from public.app_settings_internal where key = 'teacher_nome' limit 1),
  'Professor responsável não informado'
)
where btrim(teacher) = '';

alter table public.classes
  alter column teacher set default 'Professor responsável não informado';

alter table public.classes
  drop constraint if exists classes_teacher_required;

alter table public.classes
  add constraint classes_teacher_required check (char_length(btrim(teacher)) > 0);
...
begin
  ...
  insert into public.classes (id, name, teacher, academic_year, audience_id, audience_label, group_type, year_segment)
  select
    item->>'id',
    item->>'nome',
    coalesce(nullif(btrim(item->>'professor'), ''), 'Professor responsável não informado'),
    coalesce(item->>'ano', ''),
    coalesce(item->>'audienceId', ''),
    coalesce(item->>'audienceLabel', ''),
    coalesce(item->>'groupType', ''),
    coalesce(item->>'yearSegment', '')
  ...
  select item->>'id', item->>'nome', nullif(item->>'turma', ''), coalesce(item->>'matricula', ''), coalesce(item->>'status', 'Ativo') from jsonb_array_elements(coalesce(payload->'students', '[]'::jsonb)) item;
  ...
  select item->>'id', item->>'titulo', coalesce(item->>'subject', ''), coalesce(item->>'audienceId', ''), coalesce(item->>'audienceLabel', ''), coalesce(item->>'groupType', ''), coalesce(item->>'yearSegment', ''), coalesce((item->>'quantidadeQuestoes')::integer, 0), coalesce(item->'alternativas', '[]'::jsonb), coalesce(item->>'data', ''), coalesce(item->>'codigo', ''), coalesce(item->>'templateVersion', 'PS-CARD-1') from jsonb_array_elements(coalesce(payload->'exams', '[]'::jsonb)) item;
