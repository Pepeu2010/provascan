-- Keep the SECURITY DEFINER search path restricted and qualify the pgcrypto
-- function explicitly: Supabase installs pgcrypto in the extensions schema.
...
begin
  ...
  select item->>'id', item->>'nome', coalesce(item->>'professor', ''), coalesce(item->>'ano', ''), coalesce(item->>'periodo', ''), coalesce(item->>'audienceId', ''), coalesce(item->>'audienceLabel', ''), coalesce(item->>'groupType', ''), coalesce(item->>'yearSegment', '') from jsonb_array_elements(coalesce(payload->'classes', '[]'::jsonb)) item;
