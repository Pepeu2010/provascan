-- Supabase's safe-update guard rejects unconditional DELETE statements, even
-- inside this controlled state-replacement RPC. The explicit predicate keeps
-- the atomic replacement semantics while making the intent accepted.
...
begin
  ...
  insert into public.exams (id, title, subject, audience_id, audience_label, group_type, year_segment, question_count, alternatives, exam_date, code, template_version, sections)
  select item->>'id', item->>'titulo', coalesce(item->>'subject', ''), coalesce(item->>'audienceId', ''), coalesce(item->>'audienceLabel', ''), coalesce(item->>'groupType', ''), coalesce(item->>'yearSegment', ''), coalesce((item->>'quantidadeQuestoes')::integer, 0), coalesce(item->'alternativas', '[]'::jsonb), coalesce(item->>'data', ''), coalesce(item->>'codigo', ''), coalesce(item->>'templateVersion', 'PS-CARD-1'), coalesce(item->'sections', '[]'::jsonb) from jsonb_array_elements(coalesce(payload->'exams', '[]'::jsonb)) item;
