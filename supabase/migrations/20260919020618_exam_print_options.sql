-- Preferências visuais são somente da prova impressa; não alteram questões,
-- gabarito, cartão-resposta ou a geometria usada pelo OMR.
alter table public.exams
  add column if not exists print_options jsonb not null default '{"template":"institucional","typeface":"limpa","size":"normal","alternativeLayout":"lista"}'::jsonb;

alter table public.exams drop constraint if exists exams_print_options_object_check;
alter table public.exams add constraint exams_print_options_object_check
  check (jsonb_typeof(print_options) = 'object');

-- Publicações passam a gravar as preferências junto com o restante da prova
-- na mesma transação já usada para questões, gabarito e atribuições.
...
begin
  ...
  insert into public.exams (
    ...
    print_options, creator_id, source_type, status, original_file_name,
    ...
    nullif(v_exam->>'estimated_duration', '')::integer,
    coalesce(v_exam->'print_options', '{"template":"institucional","typeface":"limpa","size":"normal","alternativeLayout":"lista"}'::jsonb),
    v_exam->>'creator_id', coalesce(v_exam->>'source_type', 'manual'), v_exam->>'status',
