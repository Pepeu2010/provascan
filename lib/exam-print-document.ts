...
function printTokens(options: ExamPrintOptions) {
  const accent = options.template === "classico" ? "#7c2d12" : options.template === "compacto" ? "#0f766e" : "#4338ca";
...
function createAnswerSheet(exam: TeacherExam, options: ExamPrintOptions) {
  ...
  const details = [exam.subject, exam.audienceLabel, formatDate(exam.examDate)].filter(Boolean).map(escapeHtml).join(" · ");
  return `<main class="answer-card answer-card--${options.template}" style="${printTokens(options)}"><header class="answer-card__header"><span class="answer-card__brand">PROVASCAN · CARTÃO-RESPOSTA</span><h1>${escapeHtml(exam.title || "Prova")}</h1>${details ? `<p>${details}</p>` : ""}<div><span>Aluno(a):</span><i></i><span>Turma:</span><i></i></div></header><p class="answer-card__instruction">Marque uma única alternativa por questão.</p><div class="answer-card__caption">RESPOSTAS <span>Preencha completamente a bolha escolhida.</span></div><section class="answer-card__grid">${legends}${rows}</section><footer>Cartão de leitura ProvaScan · use caneta azul ou preta e não dobre esta folha.</footer></main>`;
}
