...
export type AssignedScopeRow = { class_id: string; subject_id: string | null; active: boolean; archived_at: string | null };
...
export function selectActiveAssignedExamClasses(
  ...
  for (const assignment of assignments) {
    ...
    if (!scopes.some((scope) => scopeAllowsExam(scope, assignment.class_id, exam.subject_id))) continue;
