...
const classSchema = z.object({ id, nome: text, professor: z.string().trim().max(500), ano: z.string().max(80), periodo: z.string().max(80), audienceId: z.string().max(120).optional(), audienceLabel: z.string().max(200).optional(), groupType: z.string().max(40).optional(), requiresManualGrouping: z.boolean().optional(), yearSegment: z.string().max(20).optional() }).strict();
